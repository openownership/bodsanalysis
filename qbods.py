...
def bodsInfo(data):
    ...
    """
    out = data.count(0).to_frame().rename(columns = {0:'Number non-missing entries'})
    cond = out.index.str.contains('_link')
    out = out.drop(out[cond].index.values)
    out['Proportion non-missing entries'] = out['Number non-missing entries']/len(data)
    out = out.round(2)
    return(out)
...
def q111(ownershipOrControlInterest,ownershipOrControlStatement):
    ...
    df = pd.merge(ownershipOrControlStatement['_link'],
            ownershipOrControlInterest[['_link_ooc_statement','type','beneficialOwnershipOrControl']],
            how = 'left',
            left_on = '_link',
...
def q121(ownershipOrControlInterest,ownershipOrControlStatement,personStatement,entityStatement):
    ...
    ooci_int = ownershipOrControlInterest[['_link_ooc_statement','beneficialOwnershipOrControl']].replace({True: 1,False: 0})
    interestSums = ooci_int.groupby('_link_ooc_statement').agg(
        beneficialOwnershipOrControl = pd.NamedAgg(column='beneficialOwnershipOrControl',aggfunc=sum)
    )
    interestSums[interestSums > 1] = 1
    interestSums['beneficialOwnershipOrControl'] = interestSums['beneficialOwnershipOrControl'].replace({1:True,0:False})
    ...
    ooc_es_ooci[['entityType','beneficialOwnershipOrControl']].rename(columns = {'entityType': 'ownerType'})   
    )

    #Crosstab of person types of with beneficial ownership
    ownerCounts = pd.crosstab(ooc_ps_es_ooci['ownerType'].fillna('Missing'),ooc_ps_es_ooci['beneficialOwnershipOrControl'].fillna('Missing'),margins = True)
    ...
    out = sortCounts(out).rename(columns = {True:'BO interests',False:'Non-BO interests','Missing':'BO data missing'})
    out.index.name = 'Interested party type'
    #Barplot
    ax = out.drop(columns = ['Total']).drop(index = ['Total']).plot.barh(stacked = True,color = cols)
    ax.set(ylabel = None,xlabel = "Number of OOC statements")
    fig = ax.get_figure()
    plt.close()
...
def q122(ownershipOrControlInterest,ownershipOrControlStatement,entityStatement):
    ...
    ownershipOrControlStatement[['_link','subject_describedByEntityStatement','interestedParty_describedByEntityStatement']],
    how = 'left',
    ...
    if len(df) > 0:
        df = pd.merge(df,
        entityStatement[['statementID','incorporatedInJurisdiction_name']],
        how = 'left',
...
def q131(ownershipOrControlInterest,ownershipOrControlStatement):
    ...
    # Graph
    ax = out.drop(labels = ['Total']).drop(columns = ['Total']).plot.barh(stacked = True,color = cols)
    ax.set(ylabel = None,xlabel = "Number of OOC statements")
    fig = ax.get_figure()
    plt.close()
...
def q132(ownershipOrControlInterest,ownershipOrControlStatement):
    ...
    df = pd.merge(ownershipOrControlStatement['_link'],
        ownershipOrControlInterest[['_link_ooc_statement','interestLevel','beneficialOwnershipOrControl']],
        how = 'left',
        left_on = '_link',
        right_on = '_link_ooc_statement')
    ...
    out = out.sort_values(by = 'All',ascending = False).rename(columns = {False:'False',True:'True'})
    out = sortCounts(out)


    #Plot
    ax = out.drop(labels = ['Total']).drop(columns = ['Total']).plot.barh(stacked = True)
    ax.set(ylabel = None,xlabel = "Number of OOC statements")
    fig = ax.get_figure()
    plt.close()
    return([out,fig])
...
def q211(entityStatement):
    ...
    Returns: a list containing i) a pandas dataframe of counts, and ii) a barplot of the same counts
    """ 
    d = {'Name': [sum(entityStatement['name'].notna()),sum(entityStatement['name'].isna())]}
...
def q212(entityStatement,entityIdentifier):
    ...
    es_subset = entityStatement.assign(identifiersPresent = entityStatement['_link'].isin(entityIdentifier['_link_entity_statement']).tolist())[['identifiersPresent','entityType']]
    out = pd.crosstab(es_subset['entityType'].fillna('Missing'),es_subset['identifiersPresent'].fillna('Missing'),margins = True)
    ...
    #Add missing codelists and fill with zero
    out = out.reindex(index = alltypes,fill_value = 0).rename(columns = {True:'True',False:'False'})
    out = sortCounts(out)
...
def q213(ownershipOrControlInterest,ownershipOrControlStatement,entityStatement):
    ...
    #Crosstab of person types of with beneficial ownership
    ownerCounts = pd.crosstab(ooc_es_ooci['entityType'].fillna('Missing'),ooc_es_ooci['beneficialOwnershipOrControl'].fillna('Missing'),margins = True)
    ...
    #Barplot
    ax = out.drop(columns = ['Total'],index = ['Total']).plot.barh(stacked = True,color = cols)
    ax.set(ylabel = None,xlabel = "Number of OOC statements")
    fig = ax.get_figure()
    plt.close()
...
def q214(ownershipOrControlStatement,entityStatement):
    ...
    outplot['jurisdictionOther'] = np.where(outplot['Total'] > 500,outplot.index,'Other')
    outplot = outplot.groupby(['jurisdictionOther']).sum().sort_values(by = ['Total'],ascending = False)
    ax = outplot.drop(columns = ['Total'],index = ['Total']).plot.barh(stacked = True)
    ax.set(ylabel = None,xlabel = "Number of OOC statements")
    fig = ax.get_figure()
    plt.close()
...
def q221(entityStatement):
    ...
    unspecDescription = unspecDescription[['unspecifiedEntityDetails_reason','descriptionProvided']]
    unspecDescription = unspecDescription.groupby(['unspecifiedEntityDetails_reason']).sum()
    ...
    #Add missing codelists and fill with zero
    out = out.reindex(index = reasontypes,columns = etypes+['descriptionProvided'],fill_value = 0)
    out = sortCounts(out)

    #Barplot
    ax = out.drop(columns = ['Total','Missing','descriptionProvided'],index = ['Total','Missing']).plot.barh(stacked = True,color = cols)
    ax.set(ylabel = None,xlabel = "Number of OOC statements")
    fig = ax.get_figure()
    plt.close()
...
def q222(personStatement):
    ...
    unspecDescription = unspecDescription[['unspecifiedPersonDetails_reason','descriptionProvided']]
    unspecDescription = unspecDescription.groupby(['unspecifiedPersonDetails_reason']).sum()
    ...
    #Add missing codelists and fill with zero
    out = out.reindex(index = reasontypes,columns = ptypes+['descriptionProvided'],fill_value = 0)
    out = sortCounts(out)

    #Barplot
    ax = out.drop(columns = ['Total','descriptionProvided'],index = ['Total','Missing']).plot.barh(stacked = True)
...
def q223(entityStatement,personStatement):
    ...
    if 'unspecifiedEntityDetails_description' in list(entityStatement):
        esout = esout + entityStatement[entityStatement['unspecifiedEntityDetails_description'].notna()]['unspecifiedEntityDetails_description'].unique().tolist()
        esout = ['unspecified entity description: ' + item for item in esout]
    if 'unspecifiedPersonDetails_description' in list(personStatement):
        psout = psout + personStatement[personStatement['unspecifiedPersonDetails_description'].notna()]['unspecifiedPersonDetails_description'].unique().tolist()
        psout = ['unspecified person description: ' + item for item in psout]

    out = '\n'.join([str(len(esout)+len(psout)) + ' statements detected ---']+esout+psout)
    return(print(out))
...
def q224(personStatement,minimumAge,maximumAge):
    ...
    psdate['birthDate'] = psdate['birthDate'].where(psdate['birthDate'] < now, psdate['birthDate'] -  np.timedelta64(100, 'Y'))
    psdate['age'] = (now - psdate['birthDate']).astype('<m8[Y]')

    d = {'Count': [sum((psdate['age'] >= minimumAge) & (psdate['age'] <= maximumAge)),
    sum((psdate['age'] < minimumAge) | (psdate['age'] > maximumAge)),
    sum(psdate['age'].isna()),
    len(psdate)]}
...
def q311(personStatement,personNationalities,personIdentifiers,personNames,declaringCountry):
    ...
    ps_pnat['identifiers'] = ps_pnat['_link'].isin(personIdentifiers['_link_person_statement'])
    ps_pnat['name'] = ps_pnat['_link'].isin(personNames['_link_person_statement'])
    out = ps_pnat[['code','birthDate','identifiers','name']].groupby(['code']).sum().transpose()
    meanout = ps_pnat[['code','birthDate','identifiers','name']].groupby(['code']).mean().transpose().round(2)
    out[declaringCountry] = out[declaringCountry].astype(str).str.cat(meanout[declaringCountry].astype(str),sep = ', ')
    out['other'] = out['other'].astype(str).str.cat(meanout['other'].astype(str),sep = ', ')
    out = sortCounts(out)
...
def q312(personIdentifiers):
    ...
    out = personIdentifiers['scheme'].fillna('Missing').value_counts().to_frame()
...
def q313(personNationalities):
    ...
    out = personNationalities['name'].fillna('Missing').value_counts().to_frame()

    out = sortCounts(out)

    #Barplot
    outplot = out.copy(deep = True)
    outplot['nameOther'] = np.where(outplot['name'] > 500,outplot.index,'Other')
    outplot = outplot.groupby(['nameOther']).sum().sort_values(by = ['name'],ascending = False)
...
def q314(personStatement):
    ...
    out = out[['birthDecadeStart','birthDecadeEnd','birthYear']].dropna().astype(int)
    out['birthDecade'] = out['birthDecadeStart'].astype(str).str.cat(out['birthDecadeEnd'].astype(str),sep = '-')
    finalout = out['birthDecade'].value_counts(sort = False).to_frame().sort_index()
    #Barplot
...
def q315(personTaxResidencies):
    ...
    """
    out = personTaxResidencies['name'].fillna('Missing').value_counts().to_frame()
    out = sortCounts(out)
...
def q321(entityIdentifiers):
    ...
    out = entityIdentifiers['scheme'].fillna('Missing').value_counts().to_frame()
    out = sortCounts(out)

    #Barplot
    outplot = out.copy(deep = True)
    outplot['schemeOther'] = np.where(outplot['scheme'] > 500,outplot.index,'Other')
    outplot = outplot.groupby(['schemeOther']).sum().sort_values(by = ['scheme'],ascending = False)
...
def q331(ownershipOrControlStatement,samplesize = 100):
    ...
    el = pd.DataFrame({'subject': subject,'interestedParty':interestedParty}).dropna()
    ...
    newnodes = list(set([node for component in components for node in component]))
    ...
    for component in components:
        ...
        numberEntities.append(sum([node in subjectList+ipEntityList for node in component]))
        numberPersons.append(sum([node in ipPersonList for node in component]))

    out = pd.DataFrame({'numberNodes':numberNodes,'numberEntities':numberEntities,'numberPersons':numberPersons}).describe()
    out = out.loc[['mean','min','max']]

    fig = plt.figure()
    ax = nx.draw_networkx(G,pos = nx.spring_layout(G),node_size = 6,width = 0.1,with_labels = False,node_color = cols[0])
    plt.close()
...
def q332(ownershipOrControlInterests,ownershipOrControlStatement):
    ...
    for i in range(len(df_indirect.index)):
      ...
      if len(sub_data.index) > 0:
        components = df_indirect['componentStatementIDs'].tolist()[i].split(',')
    ...
    out = pd.DataFrame(sub_out,index = idx,columns = ['% indirect OOC statements'])
...
def q333(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  oocComponent = ownershipOrControlStatement['isComponent'].copy(deep = True).to_frame()
  ...
  out = pd.crosstab(allComponent['statementType'].fillna('Missing'),allComponent['isComponent'].fillna('Missing'),margins = True).rename(columns = {True:'True',False:'False'})
  out = sortCounts(out)
...
def q334(ownershipOrControlInterest):
  ...
  if 'startDate' in list(ownershipOrControlInterest):
    out = ownershipOrControlInterest.copy(deep = True)[['startDate','beneficialOwnershipOrControl']].to_frame()
    out['startYear']= pd.DatetimeIndex(out['startDate']).year
    out = pd.crosstab(out['startYear'].fillna('Missing'),out['beneficialOwnershipOrControl'].fillna('Missing'),margins = True)
    out = sortCounts(out)
...
def q411(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  entityPub = entityStatement['publicationDetails_publisher_name'].copy(deep = True).to_frame()
  entityPub['statementType'] = 'entityStatement'

  personPub = personStatement['publicationDetails_publisher_name'].copy(deep = True).to_frame()
  personPub['statementType'] = 'personStatement'

  oocPub = ownershipOrControlStatement['publicationDetails_publisher_name'].copy(deep = True).to_frame()
  oocPub['statementType'] = 'ownershipOrControlStatement'

  allPub = entityPub.append(personPub).append(oocPub)
  allPub = allPub.rename(columns = {'publicationDetails_publisher_name':'publisherName'})

  out = pd.crosstab(allPub['publisherName'].fillna('Missing'),allPub['statementType'].fillna('Missing'),margins = True)
...
def q412(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  entityPub = entityStatement['publicationDetails_publicationDate'].copy(deep = True).to_frame()
  entityPub['statementType'] = 'entityStatement'

  personPub = personStatement['publicationDetails_publicationDate'].copy(deep = True).to_frame()
  personPub['statementType'] = 'personStatement'

  oocPub = ownershipOrControlStatement['publicationDetails_publicationDate'].copy(deep = True).to_frame()
  oocPub['statementType'] = 'ownershipOrControlStatement'

  allPub = entityPub.append(personPub).append(oocPub)
  allPub = allPub.rename(columns = {'publicationDetails_publicationDate':'publicationDate'})
  allPub['publicationYear']= pd.DatetimeIndex(allPub['publicationDate']).year

  out = pd.crosstab(allPub['publicationYear'].fillna('Missing'),allPub['statementType'].fillna('Missing'),margins = True)
  out = sortCounts(out)
...
def q511(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  entityPub = entityStatement['publicationDetails_license'].copy(deep = True).to_frame()
  entityPub['statementType'] = 'entityStatement'

  personPub = personStatement['publicationDetails_license'].copy(deep = True).to_frame()
  personPub['statementType'] = 'personStatement'

  oocPub = ownershipOrControlStatement['publicationDetails_license'].copy(deep = True).to_frame()
  oocPub['statementType'] = 'ownershipOrControlStatement'

  allPub = entityPub.append(personPub).append(oocPub)
  allPub = allPub.rename(columns = {'publicationDetails_license':'license'})

  out = pd.crosstab(allPub['license'].fillna('Missing'),allPub['statementType'].fillna('Missing'),margins = True)
...
def q611(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  entityPub = entityStatement['publicationDetails_bodsVersion'].copy(deep = True).to_frame()
  entityPub['statementType'] = 'entityStatement'

  personPub = personStatement['publicationDetails_bodsVersion'].copy(deep = True).to_frame()
  personPub['statementType'] = 'personStatement'

  oocPub = ownershipOrControlStatement['publicationDetails_bodsVersion'].copy(deep = True).to_frame()
  oocPub['statementType'] = 'ownershipOrControlStatement'

  allPub = entityPub.append(personPub).append(oocPub)
  allPub = allPub.rename(columns = {'publicationDetails_bodsVersion':'bodsVersion'})

  out = pd.crosstab(allPub['bodsVersion'].fillna('Missing'),allPub['statementType'].fillna('Missing'),margins = True)
...
def q711(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  out = pd.crosstab(allSource['source_type'].fillna('Missing'),allSource['statementType'].fillna('Missing'),margins = True)
...
def q811(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  oocSD = ownershipOrControlStatement['statementDate'].copy(deep = True).to_frame()
  oocSD['statementType'] = 'ownershipOrControlStatement'

  allSD = entitySD.append(personSD).append(oocSD)
  allSD['statementYear']= pd.DatetimeIndex(allSD['statementDate']).year.astype('Int64')

  out = pd.crosstab(allSD['statementYear'].fillna('Missing'),allSD['statementType'].fillna('Missing'),margins = True)
  out = sortCounts(out)

  ax = out.drop( index='Total',columns = 'Total').plot.barh(stacked = True,color = cols)
...
def q812(ownershipOrControlInterest,ownershipOrControlStatement):
  ...
  ownershipOrControlStatement[['_link','statementDate']],
  left_on='_link_ooc_statement',
  right_on='_link',
  how = 'left')
...
def q821(entityStatement):
  ...
  if 'foundingDate' in list(entityStatement):
    ...
    fd['foundingYear'] = pd.DatetimeIndex(fd['foundingDate']).year.astype('Int64')
  ...
  if 'dissolutionDate' in list(entityStatement):
    ...
    dd['dissolutionYear'] = pd.DatetimeIndex(dd['dissolutionDate']).year.astype('Int64')
...
def q831(entityStatement,personStatement,ownershipOrControlStatement):
  ...
  allRS['replacesStatements']= allRS['replacesStatements'].notna().replace([True,False],['Present','Missing'])

  out = pd.crosstab(allRS['replacesStatements'],allRS['statementType'],margins = True)
  out = sortCounts(out)
...
def q832(dict):
  ...
  for item in list(dict.keys()):
      df = dict[item]
      cols = list(df)
      dateCols = [col for col in cols if ('Date' in col) | ('retrievedAt' in col)]
      for col in dateCols:
          if col not in nams:
              nams.append(col)
              ...
              nEntries[nams == col] = nEntries[nams == col] + sum(df[col].notna())
  d_out = {'Date column':nams,'Number entries':nEntries,'Example 1':example1,'Example 2':example2,'Example 3':example3}
...
def q921(ownershipOrControlStatement):
    ...
    """
    out = ownershipOrControlStatement['interestedParty_unspecified_reason'].value_counts().to_frame()
    d = {'interestedParty_unspecified_reason':sum(ownershipOrControlStatement['interestedParty_describedByEntityStatement'].isna() & ownershipOrControlStatement['interestedParty_describedByPersonStatement'].isna() & ownershipOrControlStatement['interestedParty_unspecified_reason'].isna())}
